...
(function () {
  ...
  const T = (() => {
    ...
    return TEXTS[LANG.startsWith('fr') ? 'fr' : (LANG.startsWith('en') ? 'en' : 'zh')];
  })();
  ...
  function hideRecipientBlock(){
    ...
    if(recipient){
      ...
      if(block){
        ...
        block.style.display='none'; return true;
      }
    }
    return false;
  }
  function restoreRecipientBlock(){
    ...
    card.querySelectorAll('li.order-header__header-list-item, .a-column, .yohtmlc-recipient, [id^="shipToInsertionNode-"][id*="shippingAddress-"]').forEach(el=>{
      if(el.dataset && el.dataset._origDisplay !== undefined){
        el.style.display = el.dataset._origDisplay || ''; delete el.dataset._origDisplay; ok=true;
      }
    });
    return ok;
  }
  ...
  function ensureToolbar(){
    ...
    const btnHide = createPrimaryButton(hidden?T.showBlock:T.hideBlock, ()=>{
      ...
      if(!nowHidden){ hideRecipientBlock(); sessionStorage.setItem('amz_hide_recipient_block_latest','true'); btnHide.querySelector('.a-button-text').textContent=T.showBlock; toast(T.okHide); }
      else { restoreRecipientBlock(); sessionStorage.setItem('amz_hide_recipient_block_latest','false'); btnHide.querySelector('.a-button-text').textContent=T.hideBlock; toast(T.okShow); }
    });
